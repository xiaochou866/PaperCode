...
# 杨习贝_2019_近似推理_Pseudo-label neighborhood rough set-Measures and attribute reductions.pdf

def generateNeighbor(X: np.ndarray, attrSet: list[int], radius: float, decClassNum:int)-> dict:
    '''
    ...
    kmeans = KMeans(n_clusters=decClassNum, random_state=0).fit(X[:, attrSet])
    clusterLabels = kmeans.labels_  # 记录的是每一个样本被划分到每一个簇的情况

    neighbors = dict()
    n, _ = disMatrix.shape
    for i in range(n):
        tmpNei = set(np.where(disMatrix[i, :] < radius)[0]) - {i}
        # 与经典邻域粗糙集不同的地方 邻域改动的地方 使用伪标签对邻域进行了改动
        neighbors[i] = set()
        for e in tmpNei:
            if clusterLabels[e] == clusterLabels[i]:
                neighbors[i].add(e)
    return neighbors
...
def reductionUseNeighborhoodRoughSet(X: np.ndarray, Y: np.ndarray, radius: float = 0.2, stopCondition: str = "PRE"):
    ...
    if stopCondition == "PRE":  # 添加属性之后得到的正域比上一次正域中的样本数量多就认为该属性是有效的加入其中
        ...
        while True:
            nextA = A
            for a in A:
                neighbors = generateNeighbor(X, list(A - set([a])), radius, decClassNum)
                tmpScore = len(getPos(decClasses, neighbors))
        ...
        neighbors = generateNeighbor(X, list(AT), radius,decClassNum)
...
if __name__ == "__main__":
    ...
    red = reductionUseNeighborhoodRoughSet(X, Y, 0.05, "FULL")
    print(red)


    # generateNeighbor(X, [1,2,3], 0.2, 3)
